...
### B
贪心算法，每次都选择数量最多的石头会导致超时，因为需要模拟每一块石头。因此我们需要找到一个更快的算法。发现只要数量最多的石头数量不超过其余石头之和加一，就可以满足条件。只需预处理将首尾要求先去掉即可，代码如下：
...
def can_arrange(k, p, q, cnt):
    ...
    # Case 2: endpoints are the same (color p == q).
        ...
        # # The first and last inner stones cannot be color p, so we need non-p stones at both ends.
        # non_p = remaining - cnt[p]
        # if remaining == 1:
        #     if non_p < 1:
        #         return False
        # else:
